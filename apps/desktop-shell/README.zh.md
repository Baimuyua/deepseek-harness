...
桌面应用内置钉住版本的社区市场插件 `dshmarket@1.45.1`（安装路径就是一条用户自己也能跑的 `dsh plugin` 事务；版本只随应用发行升级）。在 sidecar 组装 `web` profile 之前——使市场首次启动即可用、无需再重启一次——壳会检查 `$DSH_HOME/profiles/web/package.json` 是否已带该依赖，缺失时用内嵌 runtime 的 pnpm 跑 `dsh plugin --profile web add dshmarket@1.45.1`。一次成功播种会在 profile 目录写入 `.dshmarket-embedded` 标记：依赖在 → 空操作；依赖不在但标记在 → 用户主动卸载，不再重装；安装失败只记 stderr，下次启动重试，绝不阻断应用。
